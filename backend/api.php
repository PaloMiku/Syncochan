...
switch($action) {
    ...
    case 'restore':
        ...
        if (is_dir($tmpcur)) {
            rrmdir($tmpcur);
            if (function_exists('log_update')) log_update("ajax restore: cleaned up temporary directory {$tmpcur}");
        }
    ...
    case 'get_webhook_logs':
        ...
        $logs = get_webhook_logs($limit, $offset);
        $total = get_webhook_logs_count();
        echo json_encode(['ok'=>true,'logs'=>$logs,'total'=>$total]);
        break;
    case 'clear_webhook_logs':
        $result = clear_webhook_logs();
        if (function_exists('log_update')) log_update("webhook logs cleared by {$_SESSION['user']}");
        echo json_encode(['ok'=>$result,'msg'=>$result ? 'cleared' : 'failed']);
        break;
    default:
